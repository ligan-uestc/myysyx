...
enum {
  TYPE_I, TYPE_R, TYPE_S, TYPE_B, TYPE_U, TYPE_J,
  TYPE_N, // none
};
...
/* J-type: inst[31]=imm[20], inst[30:21]=imm[10:1], inst[20]=imm[11], inst[19:12]=imm[19:12] */
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb  , I,
      R(rd) = SEXT(Mr(src1 + imm, 1) & 0xff, 8));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh  , I,
      R(rd) = SEXT(Mr(src1 + imm, 2) & 0xffff, 16));
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli , I,
      R(rd) = src1 << BITS(s->isa.inst, 24, 20));
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli , I,
      R(rd) = src1 >> BITS(s->isa.inst, 24, 20));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai , I,
      R(rd) = (word_t)((sword_t)src1 >> BITS(s->isa.inst, 24, 20)));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R,
      R(rd) = (word_t)(((int64_t)(sword_t)src1 * (int64_t)(sword_t)src2) >> 32));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R,
      R(rd) = (word_t)(((int64_t)(sword_t)src1 * (int64_t)src2) >> 32));
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R,
      R(rd) = (word_t)(((uint64_t)src1 * src2) >> 32));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R,
      if (src2 == 0) R(rd) = (word_t)-1;
      else if ((sword_t)src1 == INT32_MIN && (sword_t)src2 == -1) R(rd) = (word_t)INT32_MIN;
